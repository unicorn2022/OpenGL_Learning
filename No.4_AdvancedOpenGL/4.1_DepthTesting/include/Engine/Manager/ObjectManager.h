...
private:
    ObjectManager(){
        container1 = new Cube("container1", MaterialManager::GetInstance().depth_visualization_material, NULL);
        objects.push_back(container1);

        container2 = new Cube("container2", MaterialManager::GetInstance().depth_visualization_material, container1);
        objects.push_back(container2);
        container2->TransformTranslate({0.0f, 2.0f, 0.0f});

        nanosuit = new OBJModel("nanosuit", ShaderManager::GetInstance().depth_visualization_shader, container2);
