...
private:
    ObjectManager(){
        ...
        for (auto position : container_position) {
            container = new Cube("container", MaterialManager::GetInstance().container_material, NULL);
        ...
        for (auto position : window_position) {
            window = new Square("window", MaterialManager::GetInstance().window_material, NULL);
