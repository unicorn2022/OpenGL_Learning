...
public:
     ...
     * \param parent 父对象
     */
    OBJModel(std::string name, Shader* shader, Object* parent) : Object(name, NULL, parent) {
...
public:
    ...
    */
    virtual void RenderTick(Camera* camera, Material* choose_material = NULL) {
        for (auto mesh : meshs) 
            mesh->RenderTick(camera, choose_material);
    }
...
private:
    ...
    void ProcessMesh(aiMesh* ai_mesh, const aiScene* scene) {
        ...
        if (ai_mesh->mMaterialIndex >= 0) {
            ...
            if (shader == ShaderManager::GetInstance().phong_light_model_shader) {
                material = new PhongMaterial(shader, diffuse, specular);
            } else if (shader == ShaderManager::GetInstance().depth_visualization_shader) {
                material = new DepthVisualizationMaterial(shader);
            }
