...
public:
     ...
     * \note 着色器程序中, 变量命名必须是: `view_transform, projection_transform`
    */
    void Use(Shader* shader) {
        glm::mat4 view_transform = glm::lookAt(position, position + front, up);
...
public:
     ...
     * \param pitch 俯仰角
     */
    RoamingCamera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = -90.0f, float pitch = 0.0f) {
...
public:
     ...
     * \param far 远平面距离
     */
    RoamingCameraPerspective(float aspect, glm::vec3 position = glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = -90.0f, float pitch = 0.0f, float fov = 45.0f, float near = 0.1f, float far = 100.0f) 
